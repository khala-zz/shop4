                            ...
                            @foreach($categories_share as $index => $item)
                                ...
                                @if($item -> parent == null)
                                    <li class="menu-item-has-children menu-item-has-children-{{$index + 1}}">
                                        ...
                                        <a href="{{ url('cua-hang?category_id='.$item -> id) }}" class="{{Request::get('category_id') == $item -> id?'active-khala':''}}" >
                                         
                                            {{$item->title}} <i class="ion-ios-arrow-down khala-arrow-right"></i></span>
                                        </a>
                                        @else
                                        <a href="{{  url('cua-hang?category_id='.$item -> id)}}" class="{{Request::get('category_id') == $item -> id?'active-khala':''}}">
